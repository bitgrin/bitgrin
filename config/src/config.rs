...
pub const SERVER_CONFIG_FILE_NAME: &'static str = "bitgrin-server.toml";
...
fn get_bitgrin_path(chain_type: &global::ChainTypes) -> Result<PathBuf, ConfigError> {
	// Check if bitgrin dir exists
	let mut bitgrin_path = match dirs::home_dir() {
		Some(p) => p,
		None => PathBuf::new(),
	};
	bitgrin_path.push(BITGRIN_HOME);
	bitgrin_path.push(chain_type.shortname());
	// Create if the default path doesn't exist
	if !bitgrin_path.exists() {
		fs::create_dir_all(bitgrin_path.clone())?;
	}
	Ok(bitgrin_path)
}
...
fn check_api_secret_file(chain_type: &global::ChainTypes) -> Result<(), ConfigError> {
	let bitgrin_path = get_bitgrin_path(chain_type)?;
	let mut api_secret_path = bitgrin_path.clone();
...
pub fn initial_setup_server(chain_type: &global::ChainTypes) -> Result<GlobalConfig, ConfigError> {
	...
	if let Some(p) = check_config_current_dir(SERVER_CONFIG_FILE_NAME) {
		...
		let mut config_path = bitgrin_path.clone();
		...
		if !config_path.exists() {
			...
			default_config.update_paths(&bitgrin_path);
...
impl GlobalConfig {
	...
	pub fn for_chain(chain_type: &global::ChainTypes) -> GlobalConfig {
		...
		match *chain_type {
			...
			global::ChainTypes::Floonet => {
				defaults.api_http_addr = "127.0.0.1:18513".to_owned();
				defaults.p2p_config.port = 18514;
				defaults
					.stratum_mining_config
					.as_mut()
					.unwrap()
					.stratum_server_addr = Some("127.0.0.1:18516".to_owned());
				defaults
					.stratum_mining_config
					.as_mut()
					.unwrap()
					.wallet_listener_url = "http://127.0.0.1:18515".to_owned();
			}
			global::ChainTypes::UserTesting => {
				defaults.api_http_addr = "127.0.0.1:28513".to_owned();
				defaults.p2p_config.port = 28514;
				defaults.p2p_config.seeding_type = p2p::Seeding::None;
				defaults
					.stratum_mining_config
					.as_mut()
					.unwrap()
					.stratum_server_addr = Some("127.0.0.1:28516".to_owned());
				defaults
					.stratum_mining_config
					.as_mut()
					.unwrap()
					.wallet_listener_url = "http://127.0.0.1:28515".to_owned();
			}
	...
	pub fn update_paths(&mut self, bitgrin_home: &PathBuf) {
		...
		let mut secret_path = bitgrin_home.clone();
		...
		let mut log_path = bitgrin_home.clone();
