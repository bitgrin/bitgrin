...
pub fn genesis_main() -> core::Block {
	let gen = core::Block::with_header(core::BlockHeader {
		...
		height: 0,
		timestamp: Utc.ymd(2019, 1, 28).and_hms(21, 46, 31), // REPLACE
		prev_root: Hash::from_hex("0000000000000000002a8bc32f43277fe9c063b9c99ea252b483941dcd06e217",).unwrap(),// REPLACE
		output_root: Hash::from_hex("fa7566d275006c6c467876758f2bc87e4cebd2020ae9cf9f294c6217828d6872",).unwrap(),// REPLACE
		range_proof_root: Hash::from_hex("1b7fff259aee3edfb5867c4775e4e1717826b843cda6685e5140442ece7bfc2e",).unwrap(),// REPLACE
		kernel_root: Hash::from_hex("e8bb096a73cbe6e099968965f5342fc1702ee2802802902286dcf0f279e326bf",).unwrap(),// REPLACE
	...
	let kernel = core::TxKernel {
		...
		fee: 0,
		lock_height: 0,
		excess: Commitment::from_vec(util::from_hex("096385d86c5cfda718aa0b7295be0adf7e5ac051edfe130593a2a257f09f78a3b1".to_string(),).unwrap(),), // REPLACE
		excess_sig: Signature::from_raw_data(&[80, 208, 41, 171, 28, 224, 250, 121, 60, 192, 213, 232, 111, 199, 111, 105, 18, 22, 54,165, 107, 33, 186, 113, 186, 100, 12, 42, 72, 106, 42, 20, 67, 253, 188, 178, 228, 246,21, 168, 253, 18, 22, 179, 41, 63, 250, 218, 80, 132, 75, 67, 244, 11, 108, 27, 188,251, 212, 166, 233, 103, 117, 237,]).unwrap(), // REPLACE
	};
	let output = core::Output {
		features: core::OutputFeatures::Coinbase,
		commit: Commitment::from_vec(util::from_hex("08b7e57c448db5ef25aa119dde2312c64d7ff1b890c416c6dda5ec73cbfed2edea".to_string(),).unwrap(),), // REPLACE
		proof: RangeProof {
			plen: SINGLE_BULLET_PROOF_SIZE,
			proof: [ 147, 48, 173, 140,], // REPLACE
		},
	};
	gen.with_reward(output, kernel)
}
