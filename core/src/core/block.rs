...
use crate::consensus::{self, reward, reward_at_height, total_overage_at_height};
...
impl BlockHeader {
	...
	pub fn overage(&self) -> i64 {
		(reward_at_height(self.height).0 as i64)
			.checked_neg()
			.unwrap_or(0)
	}
	...
	pub fn total_overage(&self, _: bool) -> i64 {
...
impl Block {
	...
	pub fn verify_coinbase(&self) -> Result<(), Error> {
...

		{
			...
			let secp = secp.lock();
			let over_commit = secp.commit_value(reward(self.total_fees(), self.header.height).0)?;
