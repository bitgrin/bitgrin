...
mod wallet_tests {
	...
	use bitgrin_config::GlobalWalletConfig;
	use bitgrin_core::global;
	use bitgrin_core::global::ChainTypes;
	use bitgrin_keychain::ExtKeychain;
	use bitgrin_wallet::{LMDBBackend, WalletBackend, WalletConfig, WalletInst, WalletSeed};
	...
	pub fn config_command_wallet(
		...
		if config_file_name.exists() {
			return Err(bitgrin_wallet::ErrorKind::ArgumentError(
	...
	fn instantiate_wallet(
		...
		passphrase: &str,
		account: &str,
	) -> Result<Arc<Mutex<WalletInst<LocalWalletClient, ExtKeychain>>>, bitgrin_wallet::Error> {
	...
	fn execute_command(
		...
		arg_vec: Vec<&str>,
	) -> Result<String, bitgrin_wallet::Error> {
	...
	fn command_line_test_impl(test_dir: &str) -> Result<(), bitgrin_wallet::Error> {
		...
		let wallet1 = instantiate_wallet(config1.clone(), client1.clone(), "password", "default")?;
		bitgrin_wallet::controller::owner_single_use(wallet1.clone(), |api| {
		...
		let wallet2 = instantiate_wallet(config2.clone(), client2.clone(), "password", "default")?;
		bitgrin_wallet::controller::owner_single_use(wallet2.clone(), |api| {
